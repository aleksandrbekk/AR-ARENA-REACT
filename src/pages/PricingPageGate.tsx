...
const AccessGate = ({ onUnlock }: { onUnlock: () => void }) => {
    ...
    return (
        <motion.div
            className="flex flex-col items-center justify-center min-h-[80vh] px-4 relative z-20"
        ...
        >
            <div className="max-w-2xl w-full space-y-8">
                {/* Video Container */}
                <div className="relative pt-[56.25%] rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-zinc-900">
                    {/* Placeholder Video - Replace with actual ID */}
                    <iframe
...
// ============ ГЛАВНАЯ СТРАНИЦА ============
export function PricingPageGate() {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-black text-white relative overflow-x-hidden pt-[60px] pb-20 selection:bg-purple-500/30">
                ...
                {/* Контент с анимацией смены экранов */}
                <AnimatePresence mode="wait">
                    {!isAccessGranted ? (
                        <AccessGate key="gate" onUnlock={() => setIsAccessGranted(true)} />
                    ) : (
                        <motion.div
                            key="content"
                            className="relative z-10 px-3 sm:px-4 md:px-6 py-8 sm:py-10 md:py-12 max-w-7xl mx-auto"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
