  ...
  return (
    <Layout hideNavbar>
      ...
      >
        ...
        <div className="relative w-full h-[280px]">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-zinc-800/80 backdrop-blur-md rounded-full text-white text-sm active:scale-95 transition-transform cursor-pointer border border-white/10"
            >
              <span>Сменить</span>
        ...
        <div className="px-4 pt-4">
          ...
          >
            ...
            <button
              ...
              className="w-full h-[60px] rounded-2xl relative overflow-hidden transition-transform active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              style={{
          ...
          <div className="flex flex-col gap-3 mb-6">
            {allEquipment.map((eq) => {
              ...
              return (
                <div
                  key={eq.slug}
                  className="backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 border border-[#FFD700]/15 transition-all"
                  style={{
                  ...
                  <div>
                    {!locationPurchased ? (
                      ...
                      <button
                        ...
                        className="bg-transparent border border-[#FFD700]/70 text-[#FFD700] px-3 py-2 rounded-xl font-bold text-sm transition-all active:scale-95 active:bg-[#FFD700]/10 disabled:opacity-50 cursor-pointer hover:border-[#FFD700] hover:shadow-[0_0_15px_rgba(255,215,0,0.25)]"
          ...
          {farmState && (
            <div className="flex gap-3 mb-4">
              <div className="flex-1 bg-zinc-900/80 backdrop-blur-sm rounded-xl p-3 border border-white/5">
                <div className="text-zinc-500 text-xs mb-0.5">Баланс BUL</div>
                <div className="text-white font-bold">{farmState.balance_bul?.toLocaleString() || 0}</div>
              </div>
              <div className="flex-1 bg-zinc-900/80 backdrop-blur-sm rounded-xl p-3 border border-white/5">
                <div className="text-zinc-500 text-xs mb-0.5">Баланс AR</div>
                <div className="text-[#FFD700] font-bold">{(farmState.balance_ar || 0).toFixed(0)}</div>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Toast */}
        {toast.show && (
          <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-[fadeInDown_0.3s_ease-out]">
            <div className="flex items-center gap-3 px-6 py-4 bg-zinc-900/95 backdrop-blur-xl border border-green-500/30 rounded-2xl shadow-2xl">
              <div className="w-10 h-10 rounded-full bg-gradient-to-b from-green-400 to-green-600 flex items-center justify-center shadow-lg shadow-green-500/30">
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-[#0a0a0a]">
          ...
          <div className="absolute top-4 right-4 z-10">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-md border border-white/10 cursor-pointer active:scale-95 transition-transform"
            >
            ...
            <div className="space-y-3 pb-8">
              {farmState?.locations?.map((location) => (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl transition-all cursor-pointer ${location.is_current
                      ? 'bg-[#FFD700]/10 border border-[#FFD700]/40'
                      : location.purchased
                        ? 'bg-zinc-900/80 border border-white/5 active:scale-[0.98]'
                        : location.can_purchase
                          ? 'bg-zinc-900/80 border border-white/5 active:scale-[0.98]'
                          : 'bg-zinc-900/50 border border-white/5 opacity-50 cursor-not-allowed'
                    }`}
                >
                  <img
                    src={location.image}
                  ...
                  <div>
                    {location.is_current ? (
                      <span className="px-3 py-1 rounded-lg bg-[#FFD700]/20 text-[#FFD700] text-sm font-bold">Текущая</span>
                    ) : location.purchased ? (
                      <span className="text-white/50 text-sm">Выбрать ›</span>
                    ) : location.can_purchase ? (
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                        ...
                        className="px-3 py-1 rounded-lg border border-[#FFD700]/50 text-[#FFD700] text-sm font-bold cursor-pointer"
                      >
