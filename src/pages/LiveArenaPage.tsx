...
export function LiveArenaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Validate structure - old Edge Function generates different format
      ...
      if (hasValidStructure) {
        ...
        console.log('⚠️ Invalid or missing draw_results, regenerating...')
  ...
  const runDraw = async () => {
    ...
    for (const spin of results.semifinal.spins) {
      ...
      // Start spinning
      setRoulettePosition(prev => prev - 2000)
      await sleep(100)
      // Land on target
      setRoulettePosition(targetOffset)
      await sleep(3000)
  ...
  const renderSemifinal = () => (
    <div className="min-h-screen bg-[#0a0a0a] pt-[80px] pb-8 px-4">
      ...
      <div className="relative h-16 bg-zinc-900/80 rounded-xl overflow-hidden border border-[#FFD700]/20 mb-4">
        <img src="/icons/Cursor.png" alt="cursor" className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-8 z-10 drop-shadow-[0_0_10px_rgba(255,215,0,0.8)]" />
        <div
          className="flex items-center h-full transition-transform duration-[3s] ease-out pl-[50%]"
          style={{ transform: `translateX(${roulettePosition}px)` }}
        >
          {Array(15).fill(null).flatMap((_, repIdx) =>
            semifinalPlayers.map((t, tIdx) => (
              <div key={`${repIdx}-${tIdx}-${t.ticket_number}`} className="flex-shrink-0 w-20 h-full flex items-center justify-center border-r border-[#FFD700]/20">
                <span className="text-[#FFD700] font-bold text-lg">#{t.ticket_number}</span>
              </div>
            ))
          )}
        </div>
