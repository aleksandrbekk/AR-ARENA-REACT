...
export function LiveArenaTestPage() {
    ...
    const [_showSemifinalPrizes, _setShowSemifinalPrizes] = useState(true)

    // ===================== FINAL STATE =====================
    const [_finalScores, setFinalScores] = useState([
        { bulls: 0, bears: 0, place: null as number | null },
        { bulls: 0, bears: 0, place: null as number | null },
        { bulls: 0, bears: 0, place: null as number | null }
    ])
    const [_currentFinalPlayer, setCurrentFinalPlayer] = useState<number | null>(null)
    const [_wheelAngle, setWheelAngle] = useState(0)
    const [_wheelSpinning, _setWheelSpinning] = useState(false)
    const [_lastResult, setLastResult] = useState<'bull' | 'bear' | null>(null)
    ...
    if (mode === 'final') {
        // Mock turns and winners for demo
        const mockTurns = [
            { turn: 1, player: 0, result: 'bull' as const },
            { turn: 2, player: 1, result: 'bear' as const },
            { turn: 3, player: 2, result: 'bull' as const }
        ]
        const mockWinners = [
            { place: 1, ticket: mockFinalists3[0].ticket_number, username: mockFinalists3[0].player.name },
            { place: 2, ticket: mockFinalists3[1].ticket_number, username: mockFinalists3[1].player.name },
            { place: 3, ticket: mockFinalists3[2].ticket_number, username: mockFinalists3[2].player.name }
        ]
