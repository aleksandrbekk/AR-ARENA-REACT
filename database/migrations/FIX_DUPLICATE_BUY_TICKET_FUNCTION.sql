-- FIX: Remove duplicate buy_giveaway_ticket_v2 function with UUID signature
-- Problem: Two functions exist with same name but different types (text vs uuid)
-- PostgreSQL cannot choose between them when called with a string value

-- Step 1: Drop the UUID version of the function
DROP FUNCTION IF EXISTS buy_giveaway_ticket_v2(text, uuid, integer);

-- Step 2: Ensure the TEXT version exists with latest logic (prices support)
