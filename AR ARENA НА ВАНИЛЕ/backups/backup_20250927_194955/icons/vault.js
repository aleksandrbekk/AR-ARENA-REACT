...
function canClaimLockpick() {
    ...
    const lastClaimMidnight = new Date(lastClaimDate);
    lastClaimMidnight.setHours(0, 0, 0, 0);

    return lastClaimMidnight < lastMidnight;
}
...
function showSubscriptionButtons(needAlex, needPremium) {
    ...
    if (needAlex) {
        html += '<button id="alexBtn" class="sub-btn check" onclick="checkAlexSubscription()">🔍 Проверить подписку на Алексей Руденко</button>';
    }

    if (needPremium) {
        html += '<button id="premiumBtn" class="sub-btn check" onclick="checkPremiumSubscription()">🔍 Проверить подписку на PREMIUM NEWS</button>';
    }
...
function checkAlexSubscription() {
    ...
    btn.className = 'sub-btn subscribe';
    btn.innerHTML = '📱 Подписаться на Алексей Руденко';
    btn.onclick = () => {
        window.Telegram?.WebApp?.openTelegramLink('https://t.me/AlexRich2018');
        btn.className = 'sub-btn check';
...
function checkPremiumSubscription() {
    ...
    btn.className = 'sub-btn subscribe';
    btn.innerHTML = '📱 Подписаться на PREMIUM NEWS';
    btn.onclick = () => {
...
function giveLockpick() {
    ...
    localStorage.setItem('lastLockpickClaim', Date.now());
...
async function getLockpick() {
    ...
    button.disabled = true;
    updateButtonText(button, '⏳ Проверяю...');
...
async function checkSubscriptionAndGetLockpick() {
    ...
    button.disabled = true;
    updateButtonText(button, '⏳ Проверяю подписку...');
    button.className = 'lockpick-btn';
    ...
    const now = Date.now();
    localStorage.setItem('lastLockpickClaim', now);
...
function openChest(chestElement) {
    ...
    // Сбрасываем счетчик отмычек на 0
        ...
        instruction.innerHTML = `
            <div style="text-align: center; padding: 5px 0;">
                ...
                <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                    <img src="icons/time.png" style="width: 16px; height: 16px; opacity: 0.8;">
                    <span style="color: rgba(255, 255, 255, 0.7); font-size: 13px;">Следующая попытка через:</span>
                    <span id="nextTimer" style="color: #FFD700; font-size: 14px; font-weight: 500;"></span>
                </div>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Инициализируем кнопку получения отмычки
    const button = document.getElementById('lockpickBtn');
    if (button) {
        const span = button.querySelector('span');
        if (span) {
            span.textContent = 'Получить отмычку';
        }
    }
    ...
    // Проверяем получали ли отмычку сегодня (используем новую логику)
    if (canClaimLockpick() === false && !isAdmin) {
        // Если уже получали сегодня - сразу активируем сундуки
        const button = document.getElementById('lockpickBtn');
        const lastClaim = localStorage.getItem('lastLockpickClaim');

        if (lastClaim) {
            // Проверяем, была ли отмычка получена сегодня
            const lastClaimDate = new Date(parseInt(lastClaim));
            const now = new Date();
            ...
            if (lastClaimDay === today) {
                // Отмычка получена сегодня - активируем сундуки
                button.style.display = 'none';
                document.querySelector('.lockpick-info').style.display = 'none';
                document.getElementById('instructionText').style.display = 'block';
                enableChests();
