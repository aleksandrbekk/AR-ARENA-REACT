...
// Глобальные переменные для таймеров (для предотвращения утечек памяти)
let lockpickTimerInterval = null;
...
function showRewardResult(amount) {
    ...
    if (lockpickTimerInterval) clearInterval(lockpickTimerInterval);
    lockpickTimerInterval = setInterval(updateNextLockpickTimer, 1000);
}
...
window.addEventListener('beforeunload', () => {
    if (lockpickTimerInterval) {
        clearInterval(lockpickTimerInterval);
        lockpickTimerInterval = null;
    }
});