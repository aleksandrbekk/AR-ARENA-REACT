// console.log('✅ TASKS.JS: Скрипт начал выполнение');

try {
    // console.log('✅ TASKS.JS: Начало глобальной инициализации');

    // Инициализация Telegram WebApp
    if (window.Telegram && window.Telegram.WebApp) {
        window.Telegram.WebApp.ready();
        window.Telegram.WebApp.expand();
        // console.log('✅ TASKS.JS: Telegram WebApp инициализирован');
    } else {
        console.warn('⚠️ TASKS.JS: Telegram WebApp не найден');
    }

// Fetch с timeout для предотвращения зависания
async function fetchWithTimeout(url, options = {}, timeout = 5000) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            console.warn(`Fetch timeout после ${timeout}ms:`, url);
...
function renderDailyTasks() {
    // console.log('✅ [renderDailyTasks] Начало рендеринга ежедневных заданий');
    ...
    if (isDailyTaskCompletedToday()) {
        ...
        if (dailyTask1) {
            dailyTask1.remove();
            // console.log('✅ Удалено выполненное ежедневное задание');
        }
        return;
    }
...
function renderEducationTasks() {
    ...
    // console.log('✅ [renderEducationTasks] Образовательные задания (заглушка)');
}
...
document.addEventListener('DOMContentLoaded', function() {
    // console.log('✅ [Tasks] DOMContentLoaded сработал');

    // СИНХРОННЫЙ КОД - выполняется мгновенно
    try {
        // console.log('✅ [Tasks] Начало синхронной инициализации');

        loadUserProfile();
        // console.log('✅ [Tasks] loadUserProfile завершен');

        renderDailyTasks();
        // console.log('✅ [Tasks] renderDailyTasks завершен');

        renderEducationTasks();
        // console.log('✅ [Tasks] renderEducationTasks завершен');

        updateStats();
        // console.log('✅ [Tasks] updateStats завершен');

        // console.log('✅ [Tasks] Синхронная инициализация завершена');
    } catch (error) {
        console.error('❌ [Tasks] Ошибка синхронной инициализации:', error.message, error.stack);
        alert('Ошибка инициализации: ' + error.message);
    }

    // АСИНХРОННЫЕ ПРОВЕРКИ - в фоне, НЕ блокируют страницу
    // console.log('✅ [Tasks] Запуск фоновых проверок через 100ms');
    setTimeout(() => {
        // console.log('✅ [Tasks] Фоновые проверки запускаются');
        checkSubscriptionsInBackground();
        loadTaskStatesInBackground();
        loadQuizzesInBackground();
    }, 100);
});

// Фоновая проверка подписок (неблокирующая)
async function checkSubscriptionsInBackground() {
    try {
        // console.log('[Background] Проверка подписок...');
        await forceCheckSubscriptions();
    } catch (error) {
        console.error('[Background] Ошибка проверки подписок:', error);
    }
}

// Фоновая загрузка состояний заданий (неблокирующая)
async function loadTaskStatesInBackground() {
    try {
        // console.log('[Background] Загрузка состояний заданий...');
        await loadTaskStates();
    } catch (error) {
        console.error('[Background] Ошибка загрузки состояний:', error);
    }
}

// Фоновая загрузка квизов (неблокирующая)
async function loadQuizzesInBackground() {
    try {
        // console.log('[Background] Загрузка квизов...');
        await loadQuizzes();
    } catch (error) {
        console.error('[Background] Ошибка загрузки квизов:', error);
        const quizList = document.getElementById('quizList');
        if (quizList) {
            quizList.innerHTML = '<p style="color: rgba(255,255,255,0.6); text-align: center; padding: 20px;">Не удалось загрузить квизы</p>';
        }
    }
}

// Загрузка квизов из БД
async function loadQuizzes() {
    if (!supabaseClient) {
        console.warn('[Quizzes] Supabase не инициализирован');
        return;
    }
    ...
    if (error) {
        console.error('[Quizzes] Ошибка загрузки:', error);
        return;
    }

    const quizList = document.getElementById('quizList');
    if (!quizList) {
        console.warn('[Quizzes] Элемент quizList не найден');
        return;
    }

    if (!quizzes || quizzes.length === 0) {
        quizList.innerHTML = '<p style="color: rgba(255,255,255,0.6); text-align: center; padding: 20px;">Квизов пока нет</p>';
        return;
    }
    ...
    quizList.innerHTML = quizzes.map(quiz => `
        <div class="task-card quiz-card">
            <div class="task-content">
                <img src="icons/gamepad.png" alt="Quiz" style="width: 50px; height: 50px; margin-right: 15px; object-fit: contain;">
                <div class="task-info">
                    <div class="task-title">${quiz.title || 'Без названия'}</div>
                    <div class="task-description">${quiz.description || 'Проверьте свои знания'}</div>
                </div>
                <div class="task-reward-lux">
                    <span class="reward-glow">+${quiz.reward_ar || 0} AR</span>
                </div>
            </div>
            <button onclick="window.startQuiz('${quiz.id}')" class="task-button task-button-gradient">
                Начать квиз
            </button>
        </div>
    `).join('');

    // console.log(`[Quizzes] Загружено квизов: ${quizzes.length}`);
}

// Запуск квиза
window.startQuiz = function(taskId) {
    // console.log('[Quiz] Запуск квиза:', taskId);

    if (typeof window.launchQuiz === 'function') {
        window.launchQuiz(taskId);
    } else {
        console.error('[Quiz] Функция launchQuiz не найдена');
        alert('Система квизов не загружена');
    }
};

// Экспортируем handleSpecialTask в глобальную область
window.handleSpecialTask = async function(taskId) {
    // console.log('✅ [handleSpecialTask] Вызвана для задания:', taskId);
    return handleSpecialTask(taskId);
};
...
function loadUserProfile() {
    ...
    if (user) {
        ...
        // Баланс из window.currentUser
        const balance = window.currentUser?.balance_ar || 0;
        ...
        // Баланс из window.currentUser (тестовый режим)
        const balance = window.currentUser?.balance_ar || 0;
        ...
        if (headerBalanceEl) {
            ...
            console.warn('⚠️ Элемент headerBalance не найден');
        }
...
async function loadTaskStates() {
    ...
    try {
        ...
        // Обучение
        if (typeof tasks !== 'undefined' && tasks.learning) {
        ...
        console.error('[Tasks] Ошибка в loadTaskStates:', error);
    }
}
...
async function checkSubscription(channelId) {
    try {
        ...
        // Используем fetchWithTimeout с 5 секундным timeout
        const response = await fetchWithTimeout('https://ar-arena-check.levbekk.workers.dev', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_id: userId,
        ...
        console.error('Ошибка проверки подписки:', error);
        // Возвращаем false при ошибке, не блокируя приложение
        return false;
    }
}
...
function updateUserBalance(reward) {
    // Используем window.currentUser вместо localStorage
    const currentBalance = window.currentUser?.balance_ar || 0;
    const newBalance = currentBalance + reward;

    // Обновляем в window.currentUser
    ...
    const headerBalanceEl = document.getElementById('headerBalance');
    if (headerBalanceEl) {
        headerBalanceEl.textContent = newBalance;
    }
}

// Обновление статистики (упрощенная версия)
function updateStats() {
    // console.log('✅ [updateStats] Статистика обновлена (элементы отсутствуют в HTML)');

    // Подсчитываем для локального хранения
    let completed = 0;
    let earned = 0;

    // Спецзадания
    if (typeof tasks !== 'undefined' && tasks.special) {
    ...
    localStorage.setItem('completedTasksCount', completed);
    localStorage.setItem('earnedAmount', earned);

    // console.log(`✅ Статистика: выполнено ${completed}, заработано ${earned} AR`);
}
...
async function forceCheckSubscriptions() {
    ...
    try {
        ...
        console.error('[forceCheckSubscriptions] Ошибка:', error);
        // При ошибке скрываем спецзадания чтобы не мешать
        const specialSection = document.getElementById('specialTasksSection');
        if (specialSection) {
            specialSection.style.display = 'none';
        }
        return { alexSubscribed: false, premiumSubscribed: false };
    }
}
...
// console.log('✅ TASKS.JS: Файл полностью загружен и выполнен');

// Проверка экспорта функций
// console.log('🔍 Exported functions check:');
// console.log('  - window.handleSpecialTask:', typeof window.handleSpecialTask);
// console.log('  - window.startQuiz:', typeof window.startQuiz);

} catch (globalError) {
    console.error('❌ TASKS.JS КРИТИЧЕСКАЯ ОШИБКА:', globalError.message);
    console.error('❌ Stack trace:', globalError.stack);
    alert('Критическая ошибка в tasks.js: ' + globalError.message);
}

// Финальная проверка экспорта ПОСЛЕ catch
// console.log('🔍 FINAL CHECK - Exported functions:');
// console.log('  - window.handleSpecialTask:', typeof window.handleSpecialTask);
// console.log('  - window.startQuiz:', typeof window.startQuiz);

// Если функции НЕ экспортированы, значит был exception в try-catch
// Проверяем доступность функций для отладки
if (typeof window.handleSpecialTask === 'undefined') {
    console.error('❌ window.handleSpecialTask НЕ ЭКСПОРТИРОВАНА!');
    console.error('❌ Это означает, что произошла ошибка в try-catch блоке');
}

if (typeof window.startQuiz === 'undefined') {
    console.error('❌ window.startQuiz НЕ ЭКСПОРТИРОВАНА!');
    console.error('❌ Это означает, что произошла ошибка в try-catch блоке');
}
